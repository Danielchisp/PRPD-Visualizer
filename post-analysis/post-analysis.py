...
subfoldersName = ['Bushing cobre particulas', 'Nuevas Internas - copia', 'Metacrilato Superficiales - Volteado correctamente']
...
for i in subfolders:
    ...
    impulse_ave = np.load(os.path.join(folder, i, 'impulse_ave_final' + '.npy'))
    time_impulse = np.load(os.path.join(folder, i, 'time1' + '.npy'))
    
    main_discharges_ids = pd.read_csv(os.path.join(folder, i, 'Main HFCT.csv'))
    reverse_discharges_ids = pd.read_csv(os.path.join(folder, i, 'Reverse HFCT.csv'))
...
temporalPlotExample = np.array(data_per_voltage[0][data_per_voltage[0]['id'].isin(main_discharges_ids_per_voltage[0]['id'])]['signal'])
timeExample = np.array(data_per_voltage[0][data_per_voltage[0]['id'].isin(main_discharges_ids_per_voltage[0]['id'])]['t'])
...
for j in range(len(impulses_per_voltage)):
    ...
    time_pd = np.array(data_per_voltage[j][data_per_voltage[j]['id'].isin(main_discharges_ids_per_voltage[j]['id'])]['x'] - (delays_per_voltage[j]['t0_linear']*delays_per_voltage[0]['delay_us']/delays_per_voltage[0]['delay_samples'])[0])
    amplitude_pd = np.array(data_per_voltage[j][data_per_voltage[j]['id'].isin(main_discharges_ids_per_voltage[j]['id'])]['Vpp'])
    vpp_pd = data_per_voltage[j][data_per_voltage[j]['id'].isin(main_discharges_ids_per_voltage[j]['id'])]['Vpp']   
    fft = data_per_voltage[j][data_per_voltage[j]['id'].isin(main_discharges_ids_per_voltage[j]['id'])]['fft_values']
    time_impulse = time_impulses_per_voltage[j] - (delays_per_voltage[j]['t0_linear']*delays_per_voltage[0]['delay_us']/delays_per_voltage[0]['delay_samples'])[0]
    
    amplitude_dependence_per_kv.append(best_fit_to_impulse_curve(time_pd, amplitude_pd, time_impulse, impulses_per_voltage[j])[0])
    R2_dependence_per_kv.append(round(100*best_fit_to_impulse_curve(time_pd, amplitude_pd, time_impulse, impulses_per_voltage[j])[1],3))
    ...
    plt.plot(time_impulse, impulses_per_voltage[j]*amplitude_dependence_per_kv[-1], alpha = 1, color = palette[j])
...
fig, ax = plt.subplots(dpi = 300)
plt.plot(tensiones.iloc[0], amplitude_dependence_per_kv, marker = 'o', linewidth = 1.5, color= palette[0])
...
ax.set_ylabel('Average Time of Occurence ($\mu$s)', color=palette[0])  # Color igual que la curva
...
ax2.set_ylabel('Time Range ($\mu$s)', color=palette[1])  # Color igual que la curva
...
# Primera gráfica (eje izquierdo - azul)
line1 = ax.plot(tensiones.loc[0], vpp_mean_per_kv, marker='o', color=palette[0])[0]
ax.set_xlabel('Voltage (kV)')
...
# Segunda gráfica (eje derecho - rojo)
ax2 = ax.twinx()
line2 = ax2.plot(tensiones.loc[0], np.array(num_discharges_main)/numImpulsesTest, marker='o', color=palette[1])[0]
ax2.set_ylabel('$N_{PD}/Impulse$', color=palette[1])  # Color igual que la curva
ax2.tick_params(axis='y', labelcolor=palette[1])  # Color de los ticks
...
fig, ax = plt.subplots(dpi = 300)
temporalPlotExample = np.array(data_per_voltage[0][data_per_voltage[0]['id'].isin(antenna_ids_per_voltage[0]['id'])]['signal'])
timeExample = np.array(data_per_voltage[0][data_per_voltage[0]['id'].isin(antenna_ids_per_voltage[0]['id'])]['t'])
plt.plot(timeExample[0], temporalPlotExample[0], color= palette[0])
plt.xlabel('Time ($\mu s$)')
plt.ylabel('Voltage (V)')
plt.show()
...
for j in range(len(impulses_per_voltage)):
    ...
    time_pd = np.array(data_per_voltage[j][data_per_voltage[j]['id'].isin(antenna_ids_per_voltage[j]['id'])]['x'] - (delays_per_voltage[j]['t0_linear']*delays_per_voltage[0]['delay_us']/delays_per_voltage[0]['delay_samples'])[0])
    amplitude_pd = np.array(data_per_voltage[j][data_per_voltage[j]['id'].isin(antenna_ids_per_voltage[j]['id'])]['y'])
    vpp_pd = data_per_voltage[j][data_per_voltage[j]['id'].isin(antenna_ids_per_voltage[j]['id'])]['Vpp']   
    fft = data_per_voltage[j][data_per_voltage[j]['id'].isin(antenna_ids_per_voltage[j]['id'])]['fft_values']
    time_impulse = time_impulses_per_voltage[j] - (delays_per_voltage[j]['t0_linear']*delays_per_voltage[0]['delay_us']/delays_per_voltage[0]['delay_samples'])[0]
...
fig, ax = plt.subplots(dpi = 300)
temporalPlotExample = np.array(data_per_voltage[0][data_per_voltage[0]['id'].isin(reverse_discharges_ids_per_voltage[0]['id'])]['signal'])
timeExample = np.array(data_per_voltage[0][data_per_voltage[0]['id'].isin(reverse_discharges_ids_per_voltage[0]['id'])]['t'])
plt.plot(timeExample[0], temporalPlotExample[0], color= palette[0])
plt.xlabel('Time ($\mu s$)')
plt.ylabel('Voltage (V)')
plt.show()
...
for j in range(len(impulses_per_voltage)):
    ...
    amplitude_pd = data_per_voltage[j][data_per_voltage[j]['id'].isin(reverse_discharges_ids_per_voltage[j]['id'])]['y']
    reverse_amplitude_pd_all.append(np.mean(amplitude_pd))
    
    vpp_pd = data_per_voltage[j][data_per_voltage[j]['id'].isin(reverse_discharges_ids_per_voltage[j]['id'])]['Vpp']
    vpp_reverse.append(np.mean(vpp_pd*1000))
    
    time_pd = data_per_voltage[j][data_per_voltage[j]['id'].isin(reverse_discharges_ids_per_voltage[j]['id'])]['x'] + delays_per_voltage[j]['delay_us'][0]
    ...
    fft = data_per_voltage[j][data_per_voltage[j]['id'].isin(reverse_discharges_ids_per_voltage[j]['id'])]['fft_values']
    fft_all_reverse.append(np.mean(fft))
    
    plt.plot(time_impulses_per_voltage[j] + delays_per_voltage[j]['delay_us'][0], impulses_per_voltage[j]*gain, alpha = 1, color = palette[j])
    plt.scatter(time_pd, amplitude_pd, s = 3, label = f'{str(tensiones[j][0])} kV', color = palette[j])
    # plt.xlim(50, 200)
...
ax.set_xlabel('Voltage (kV)')
ax.set_ylabel('Minimum Time ($\mu$s)', color=palette[0])  # Color igual que la curva
ax.tick_params(axis='y', labelcolor=palette[0])  # Color de los ticks

# Segunda gráfica (eje derecho - rojo)
ax2 = ax.twinx()
line2 = ax2.plot(tensiones.loc[0], range_reverse, marker='o', color=palette[1])[0]
ax2.set_ylabel('Time Range ($\mu$s)', color=palette[1])  # Color igual que la curva
ax2.tick_params(axis='y', labelcolor=palette[1])  # Color de los ticks
...
ax.set_xlabel('Voltage (kV)')
ax.set_ylabel('Peak to Peak Voltage (mV)', color=palette[0])  # Color igual que la curva
ax.tick_params(axis='y', labelcolor=palette[0])  # Color de los ticks

# Segunda gráfica (eje derecho - rojo)
ax2 = ax.twinx()
line2 = ax2.plot(tensiones.loc[0], np.array(num_reverse)/numImpulsesTest, marker='o', color=palette[1])[0]
ax2.set_ylabel('$N_{PD}/Impulse$', color=palette[1])  # Color igual que la curva
ax2.tick_params(axis='y', labelcolor=palette[1])  # Color de los ticks
...
for j in range(len(impulses_per_voltage)):
    ...
    pd_times = data_per_voltage[j][data_per_voltage[j]['id'].isin(reverse_discharges_ids_per_voltage[j]['id'])]['x'] + delays_per_voltage[j]['delay_us'][0]
    pd_vpps = data_per_voltage[j][data_per_voltage[j]['id'].isin(reverse_discharges_ids_per_voltage[j]['id'])]['Vpp']*1000
...
for i in range(len(subfolders)):
    if i > 0:
        vpp_mean_per_frame_per_kV.append(mean_pd_per_frame[frame_num*(i):frame_num*(i+1)])
        num_PD_per_frame_per_kV.append(num_pd_per_frame[frame_num*(i):frame_num*(i+1)])
...
for i in vpp_mean_per_frame_per_kV:
    plt.plot(time_step_groups, i, label = f'{str(tensiones[vpp_mean_per_frame_per_kV.index(i)][0])} kV', color = palette[vpp_mean_per_frame_per_kV.index(i)])
...
for i in num_PD_per_frame_per_kV:
    plt.plot(time_step_groups, i, label = f'{str(tensiones[num_PD_per_frame_per_kV.index(i)][0])} kV', color = palette[num_PD_per_frame_per_kV.index(i)])
